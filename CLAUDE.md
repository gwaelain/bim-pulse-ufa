...
Дзен-канал — **dzen.ru/bimpulse**, сайт к нему привязан (файл `zen_fQqdD…H0w.html`
в корне, не удалять). Своего API для публикации у Дзена нет; из двух путей импорта
трансляция по RSS открывается только с 10 подписчиков, а кросспостинг из Telegram
доступен сразу и требует действий владельца в мессенджере. Подробности и порядок —
`e:\Project\_content\_klyuchi\bim-pulse\dzen.md`.

## Telegram-канал

Посты в @bim_pulse_ufa идут сами: очередь заметок — `content/tg/*.md`, воркер
`tools/tg_post.py` на сервере, cron `30 4,8,13 * * *` UTC (09:30/13:30/18:30 Уфа),
один пост за запуск. Анонс новой статьи всегда впереди очереди. Токен бота —
`/opt/drip/tg.env`, состояние отправленного — `/opt/drip/tg-sent.json` (вне репозитория).

Анонс статьи и заметка чередуются (`next_item`): после починки `published.json` 17.09 в хвосте
оказалось 11 непроанонсированных статей. Очередь заметок кончилась 16.09 — пополнена 025–036;
следить, чтобы не пустела (лог на сервере пишет «очередь пуста — постить нечего»).
...
- Личка @bimpulsebot — тот же консультант и **единая точка входа**: все кнопки «Написать
  в Telegram» на сайте ведут сюда с меткой `?start=<откуда>` (main/header/contacts/article/
  service/chat/faq/test); канал @bim_pulse_ufa остаётся только в подвале и `sameAs`.
  Каждый вопрос человека + ответ консультанта копируется админам с тегом `#u<id>`;
  админ отвечает человеку обычным reply на это уведомление (уходит как «Ответ команды»).
  Админ = тот, кто написал `/start` с username из `ADMIN_USERNAMES`; `/leads` — последние.
- Входящая почта `bimaip@yandex.ru` (`app/mail.py`): IMAP раз в `IMAP_POLL_SECONDS` (180),
  непрочитанные письма → админам в Telegram + заявка `source=email`; свои уведомления
  «Заявка №…» пропускаются. Тот же логин/пароль приложения, что для SMTP. Если Яндекс
  снова закроет IMAP — кнопка «Сохранить доступ» в веб-почте (память `yandex-imap-2026-limit`).
- httpx-лог переведён на WARNING: на INFO он печатал URL с токеном бота в `docker logs`.
...
- Яндекс.Метрика 111661242 (плюс старый 109103460) — в партиале `tools/partials/metrika.html`.
  Цели только через `ymGoal('имя')` (объявлен там же, шлёт в оба счётчика): `telegram_click`,
  `email_click`, `lead_form_submit`, `chat_open`, `chat_message`, `quiz_start/finish/lead`.
  Отчёты доступны только по 111661242 — прямой `ym(109103460,'reachGoal',…)` не писать.
