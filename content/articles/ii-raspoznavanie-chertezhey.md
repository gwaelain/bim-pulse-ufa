...
Распознать основную надпись, номер листа, шифр, стадию, дату ревизии — задача, которую современный OCR решает почти без ошибок. На выходе таблица: файл, шифр, лист, ревизия.
...
Проблема не в линиях, а в смысле. Векторизатор выдаёт множество отрезков. Он не знает, что вот эти две параллельные линии на расстоянии 380 мм — стена, а вот эти, тоже параллельные, — размерная линия с выносками. Без этого понимания вектор остаётся картинкой, только другого формата.
...
Поэтому честная формулировка звучит так: ИИ ускоряет подготовку данных для моделирования и проверку результата, но моделирует по-прежнему человек. Всё, что обещает иное, стоит проверять на своих чертежах, а не на демонстрационных.

## Что даёт связка на практике

Собранный вместе набор выглядит так. Архив приходит — за час получаем ведомость листов и список того, чего не хватает. Спецификации распознаются в таблицы, проверяются контрольными суммами и уходят к сметчику раньше, чем начнётся моделирование. Слои DWG размечаются полуавтоматически, типовые планы собираются быстрее. Новые ревизии сравниваются автоматически, и на модель попадают только реальные изменения.
...
Тем, кто планирует переводить документацию в модель, полезно заранее посмотреть, <a href="2d-v-bim-perevod-chertezhey.html">как устроен процесс перевода 2D в BIM</a>. А общая картина по применению языковых моделей в проектировании — в разборе <a href="ai-in-bim-practice.html">где LLM реально работают в BIM, а где нет</a>.
